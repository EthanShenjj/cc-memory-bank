...
- **快捷保存**: 聊天发送 `ss` 回车即可自动提炼近期对话要点并增量存入对应的记忆文件。
- **跨项目导入**: 输入 `/ss-memory-bank: <路径>` 可将任意项目的记忆一键合并到全局 Memory Bank。
...
| 指令 (聊天框输入) | 对应行为 |
|------------------|----------|
| **`ss`** | **快捷保存**: 自动审视最近对话，提炼要点增量写入文件，并给出简短反馈。 |
...
| **`/ss-memory-bank: <项目路径>`** | **跨项目导入**: 读取指定项目的 `.cc-memory-bank/`，智能合并到全局记忆。 |

---

## ⌨️ 进阶：配置真正的 VS Code 快捷键

虽然插件无法直接绑定系统级键盘快捷键，但你可以通过修改 VS Code 的 `keybindings.json`，实现”按一下快捷键，自动向聊天发送 `ss`”的丝滑体验！
...
配置后，你只需在聊天框内或编辑器中按下 `Cmd+Shift+M`，即可快捷触发记忆保存（发送 `ss`）！
