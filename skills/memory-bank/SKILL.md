...
### 1. 快捷保存指令 (`ss`)
- **触发条件**: 用户在聊天框中发送单独的 `ss` 字符，或发送包含 `ss` 的保存请求（如 `ss一下`）。
- **行为逻辑**:
...
## 已完成 (Completed)
- [x] 2026-05-27: 完成 cc-memory-bank 插件的基础架构设计
...
1. 先使用 `Read` 工具读取目标文件的现有内容。
2. 识别出文件原有的层级结构 (如 `# 已完成`、`## 待做`)。
3. 利用 `Edit` 工具在精确位置插入新提炼的内容，同时保留其他历史记录。如需创建新文件则使用 `Write` 工具。
